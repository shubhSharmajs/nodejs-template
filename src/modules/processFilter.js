...
filter.parseAndValidate = (request,attributes) => {
  var params = _.assign(request.params, request.body, request.query);
  ...
  for (const key in request.params.filter) {
    ...
    parsedFilterElement = filter._parseFilterElement(key, attributes[key], filterElement)
